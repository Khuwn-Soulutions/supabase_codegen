// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, implicit_dynamic_list_literal, implicit_dynamic_map_literal, inference_failure_on_collection_literal

import 'package:mason/mason.dart';

final serverpodBundle = MasonBundle.fromJson(<String, dynamic>{
  "files": [
    {
      "path": "enums/{{#enums}}{{{fileName}}}.spy.yaml{{/enums}}",
      "data":
          "e3s+IHNweS5oZWFkZXJ9fQplbnVtOiB7e2VudW1zLmZvcm1hdHRlZEVudW1OYW1lfX0Kc2VyaWFsaXplZDogYnlOYW1lCnZhbHVlczp7eyNlbnVtcy52YWx1ZXN9fQogIC0ge3sufX17ey9lbnVtcy52YWx1ZXN9fQp7ez4gc3B5LmZvb3Rlcn19",
      "type": "text",
    },
    {
      "path": "tables/{{#tables}}{{{name}}}.spy.yaml{{/tables}}",
      "data":
          "",
      "type": "text",
    },
    {
      "path": "{{~ spy.footer }}",
      "data":
          "e3sjaGFzVGFnfX0jIyBUYWc6IHt7dGFnfX17ey9oYXNUYWd9fQojIyBEYXRlOiB7e2RhdGV9fQ==",
      "type": "text",
    },
    {
      "path": "{{~ spy.header }}",
      "data":
          "IyMKIyMgR2VuZXJhdGVkIGZpbGUuIERPIE5PVCBFRElUIE1BTlVBTExZLgojIyBHZW5lcmF0ZWQgYnkge3twYWNrYWdlfX0gKHt7dmVyc2lvbn19KQojIw==",
      "type": "text",
    },
  ],
  "hooks": [],
  "name": "serverpod",
  "description":
      "Brick to generate tables and enums for Supabase Codegen Serverpod",
  "version": "0.1.0+1",
  "environment": {"mason": "^0.1.1"},
  "readme": {
    "path": "README.md",
    "data":
        "",
    "type": "text",
  },
  "changelog": {
    "path": "CHANGELOG.md",
    "data": "IyAwLjEuMCsxCgotIFRPRE86IERlc2NyaWJlIGluaXRpYWwgcmVsZWFzZS4K",
    "type": "text",
  },
  "license": {
    "path": "LICENSE",
    "data": "VE9ETzogQWRkIHlvdXIgbGljZW5zZSBoZXJlLgo=",
    "type": "text",
  },
  "vars": {},
});
